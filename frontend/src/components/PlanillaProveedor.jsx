...
const initialSpreadsheetState = { headers: [], rows: [], footerCalculations: [], columnConfigs: [] };

export default function PlanillaProveedor({
    proveedorId,
    onBack,
    ...
    const cargar = useCallback(async () => {
        if (!proveedorId) {
            setProveedor(null);
            setSpreadsheet(initialSpreadsheetState);
            setColumnFilters({});
            setLoading(false);
            return;
        }
        ...
        setError("");
        try {
            const res = await api.get(`/proveedores/${proveedorId}`);
            setProveedor(res.data);
            setSpreadsheet(normalizeSpreadsheet(JSON.parse(res.data.data)));
            setColumnFilters({});
        } catch (e) {
            console.error("Error al cargar detalle", e);
            setError("No pudimos cargar la planilla del proveedor");
        } finally {
            setLoading(false);
        }
    }, [proveedorId]);
    ...
    const guardar = async () => {
        ...
        try {
            await api.put(`/proveedores/${proveedorId}`, {
    ...
    if (!proveedorId) {
        return <div style={{ padding: 40 }}>Selecciona un proveedor para ver la planilla.</div>;
    }

    if (loading) return <div style={{ padding: 40 }}>Cargando planilla...</div>;
    if (!proveedor) return <div style={{ padding: 40 }}>No se encontró el proveedor.</div>;

    const title = customTitle || proveedor.nombre;
        ...
        <div className="planilla-proveedor" style={{ padding: "30px 40px", height: "100%", overflowY: "auto", ...containerStyle }}>
            ...
            {(!hideBackButton && onBack) && (
                <div style={{ marginBottom: 10 }}>
                    <button onClick={onBack} style={{ background: "transparent", border: "none", color: "#64748b", cursor: "pointer", fontSize: "0.9rem", padding: 0 }}>
                        {backLabel}
                    </button>
                </div>
            )}

            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 30, gap: 12 }}>
                <div>
                    <h1 style={{ margin: 0, color: "#1e293b", fontWeight: 800 }}>{title}</h1>
                    <p style={{ margin: "6px 0 0 0", color: "#94a3b8" }}>{subtitle}</p>
                    {extraHeaderContent}
                </div>
            ...
            {error && (
                <div style={{ background: "#fee2e2", border: "1px solid #fecaca", color: "#b91c1c", padding: "10px 14px", borderRadius: 8, marginBottom: 20 }}>
                    {error}
                </div>
            )}
            ...
            {showFooterNote && (
                <p style={{ marginTop: 20, color: "#94a3b8", fontSize: "0.85rem", fontStyle: "italic" }}>
                    * Las celdas se calculan automáticamente si contienen solo números. Puedes agregar tantas columnas y filas como necesites.
                </p>
            )}
